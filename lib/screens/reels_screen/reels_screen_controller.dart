...
class ReelsScreenController extends BaseController {
  RxMap<int, CachedVideoPlayerPlusController> videoControllers = <int, CachedVideoPlayerPlusController>{}.obs;
  ...
  void _disposeControllerAtIndex(int index) {
    if (reels.length > index && index >= 0) {
      final CachedVideoPlayerPlusController? controller = videoControllers[index];
      if (controller != null) {
  ...
  Future<void> disposeAllController() async {
    for (var controller in videoControllers.values) {
      controller.pause(); // Pause the controller before disposing
